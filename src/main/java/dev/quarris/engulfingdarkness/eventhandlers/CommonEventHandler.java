...
public class CommonEventHandler {
    ...
    @SubscribeEvent
    public static void playerTick(tick.PlayerTickEvent event) {
        if (event.phase == TickEvent.Phase.START) {
            event.getEntity().getCapability(ModRef.Capabilities.DARKNESS).ifPresent(IDarkness::tick);
        }
    }
    ...
    public static void addHoverText(ItemTooltipEvent event) {
        FlameConfigs.LIGHT_BRINGERS.keySet().forEach(item -> {
            if (event.getItemStack().equals(item)) {// was .is(item)
